...
- **면 사다리(surface ladder)** — 무언가가 그 위에 놓이는 배경들의 순서. `bg.canvas` → `bg.surface` → `bg.subtle`·`bg.inset`·`bg.overlay`. **칸 수가 이름 수보다 적다** — `bg.subtle`과 `bg.inset`은 한 값이고, 다크에서는 `bg.overlay`·`bg.neutral.soft`까지 넷이 같다. 의도된 별칭이며, 토큰이 `$extensions["design.massive.sameValue"]`로 스스로 선언하고 `tokens:lint` B16이 그 선언을 실제 상태와 일치시킨다. DS는 `subtle`보다 더 파인 면을 제공하지 않는다 — 면을 더 가르는 일은 값이 아니라 테두리·그림자·여백이 한다([#337](https://github.com/flameware/massive-design/issues/337)).
- **채움 사다리(fill ladder)** — 면 위에 얹히는 덩어리들의 순서. 패밀리마다 `soft`(step 3) → `muted`(라이트 7·다크 6) → `solid`(step 9). **면 사다리와 팔레트 step 3에서 만난다** — 그래서 `*.soft`는 `bg.subtle`·`bg.inset` 위에서 1.00:1이고, 이것은 결함이 아니라 "면과 구별되지 않을 만큼 조용한 틴트"라는 `soft`의 정의다.
- **면으로 읽히는 채움** — 채움이되 면처럼 칸으로 읽혀야 하는 자리. 미터의 칸이 여기다. 컨트롤 어포던스가 아니므로 WCAG 1.4.11의 3:1을 빚지지 않지만 **그래도 보여야 한다** — 그래서 DS가 하한을 따로 진다: 앉는 면에 **1.35:1**, 그 위 전경은 **`fg.default` 고정**(유채 전경은 이 단계 위에서 AA를 못 넘는다). `bg.<family>.muted`가 이 자리이고 `tokens:contrast`의 `fill` 게이트가 5패밀리 × 5면 × 2모드를 잰다([#336](https://github.com/flameware/massive-design/issues/336)·[#337](https://github.com/flameware/massive-design/issues/337)).
...
- **컨트롤 높이(control height)** — 한 줄에 나란히 서는 컨트롤이 같은 `size` 이름에서 함께 갖는 **겉 높이**. 척도는 `sm` 32 · `md` 36 · `lg` 40(px)이다. 여기 드는 것은 Input · Select 트리거 · Combobox 입력 · NumberField · Button · Toggle, 그리고 **ToggleGroup의 겉**이다 — 묶음 컨트롤은 판이 이 높이를 지고 안의 항목은 판의 여백과 테두리만큼 작아진다(그룹 안 Toggle ≠ 낱개 Toggle). 밑줄 Tabs처럼 필드와 한 줄에 서지 않는 컨트롤은 이 척도 밖이다. **히트 영역과 다른 축이다** — 그쪽은 누름을 받는 면의 하한이고, 이쪽은 나란히 섰을 때 그려지는 면이 맞는가다.
  _피할 말_: 컨트롤 크기(너비까지 섞인다), 밀도(전역 모드가 아니다 — ADR-0023 §11)
