...
// massive-design fork: every component's stories live under ONE shared title
// ("Components/Manifest references" - see apps/storybook/stories/Components.stories.tsx),
// generated per-component from a manifest catalog. titleParts's segment-scan
// can't discover the component here since the title itself carries no
// component name - the component identity is each STORY's own name (Storybook
// derives it from the export key, e.g. export const AlertDialog -> name
// "Alert Dialog"), which already matches a real package export 1:1. So:
// try normal title-based grouping first (keeps this fork generic for any
// OTHER story files in the repo that follow the standard one-title-per-
// component convention), and only when that yields a name that isn't a real
// export (and isn't titleMap-excluded), fall back to matching the entry's
// OWN name against exportedSet - one component per story in that case.
...
export async function resolveStorybook(ctx) {
  ...
  if (sbStatic) {
    ...
    // massive-design fork: squash(exportedSet member) -> exact export name,
    // for the per-story fallback below (see file-header comment).
    const exportSquash = new Map();
    if (exportedSet) for (const n of exportedSet) exportSquash.set(squash(n), n);
    ...
    for (const e of idxEntries) {
      ...
      let { name: compName, group } = titleParts(e.title, titleMap, exportedSet);
      if (compName === null) continue; // titleMap {Name: null} = excluded
      // Title-based match failed to land on a real export - one shared title
      // holding many components' stories (each story name IS the component).
      // Fall back to the story's own name before giving up on it.
      if (!exportedSet?.has(compName)) {
        const viaStoryName = exportSquash.get(squash(e.name));
        if (viaStoryName) compName = viaStoryName;
      }
