...
// when nothing else names a types root, and `exports["."]` resolves on disk
// to a .ts/.tsx file, use ITS directory - ts-morph type-checks .ts/.tsx
// exactly as it does .d.ts (skipLibCheck only affects .d.ts libs it doesn't
// own).
export function resolveSourceEntry(pkgDir, pkgJson) {
  const exp = pkgJson.exports?.['.'];
  const rel = typeof exp === 'string' ? exp : typeof exp === 'object' ? (exp.import ?? exp.default ?? exp.require) : null;
  if (typeof rel !== 'string' || !/\.tsx?$/.test(rel)) return null;
...
export function findTypesRoot(pkgDir, pkgJson) {
  ...
  const srcEntry = resolveSourceEntry(pkgDir, pkgJson);
  if (srcEntry) return dirname(srcEntry);
  return pkgDir;
}
...
function projectFor(pkgDir, typesRoot) {
  ...
  // massive-design fork: no .d.ts anywhere (see findTypesRoot comment) - fall
  // back to the real TS source entry resolveSourceEntry names, instead of
  // the upstream default 'index.d.ts' (which never exists on this package).
  const srcEntry = resolveSourceEntry(pkgDir, pj);
  const entry = join(pkgDir, (pubEntry && existsSync(join(pkgDir, pubEntry)) ? pubEntry : null) || pj.types || pj.typings
    || (srcEntry ? srcEntry.slice(pkgDir.length + 1) : null) || 'index.d.ts');
  ...
  // massive-design fork: zero .d.ts (source-distributed package) - load the
  // real .ts/.tsx tree instead so getExportedDeclarations() can walk the
  // `export * from` chain (index.ts -> index.gen.ts -> components/ui/*.tsx)
  // fully, rather than relying on ts-morph's on-demand resolution of files
  // never added to the project.
  if (project.getSourceFiles().length === 0 && srcEntry) {
    project.addSourceFilesAtPaths([`${root}/**/*.{ts,tsx}`, `!${root}/**/node_modules/**`, `!${root}/**/*.test.{ts,tsx}`]);
    console.error(`  [DTS] no .d.ts — parsed ${project.getSourceFiles().length} .ts/.tsx source files from ${root} instead (source-distributed package)`);
  }
