...
function header(mode) {
  return `// ⚙ 생성물 — scripts/manifest.mjs. 손대지 말 것.\n` +
    `// Components page 카탈로그 배치 ${mode === "sync" ? "정규화" : "검사"}.\n` +
    `// use_figma의 code 파라미터에 그대로 넣는다. IIFE로 감싸지 말 것.\n`
}

export function emitCatalogLayout(entries, mode) {
  if (!['check', 'sync'].includes(mode)) throw new Error(`알 수 없는 카탈로그 배치 모드: ${mode}`)
...
export const catalogLayout = { x: X, startY: START_Y, gap: GAP }
