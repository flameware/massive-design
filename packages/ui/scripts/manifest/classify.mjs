...
export const MODIFIER_POLICY = new Map([
  ...
  // Sidebar rail의 포인터 히트 영역(#111 결정 4, #230). `after:`가 이미 hover 강조선을
  // 그리고 있어 같은 의사 요소를 히트 영역에도 쓰면 강조선 자체가 넓어져 시각이
  // 움직인다 — 그래서 `before:`를 쓴다. 뜻은 `after`와 같다: 투명한 의사 요소라
  // Figma에 그릴 것이 없다
  ["before", "ignore:포인터 히트 영역 — 투명한 의사 요소라 Figma에 그릴 것이 없다(#124)"],
