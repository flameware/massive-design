...
Toast의 스와이프를 [ADR-0005](../../../../../docs/adr/0005-inherited-dismiss-gestures.md)가 `gestures`로 얕게 계약하면서, 같은 판정이 **컨트롤 제스처는 이 계약 밖이다**라고 적었다. 그 문장은 참이다 — Slider·Scroll Area·Carousel의 드래그는 표면을 없애지 않고 키보드 동등 경로가 이미 각 계약 안에 있다. 그런데 "밖"이 **"어디에도 없음"**을 뜻하게 방치됐다.
