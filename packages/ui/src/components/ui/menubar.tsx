 ...
 * 그리는 것은 `checked` 구성 상태이지 별도 노드가 아니다. 그래서 파트로 열지 않는다:
 * 껍데기를 노드로 세우면 두 항목이 같은 클래스를 갖게 되어 파생 채널이 가르지 못한다
 * (Select의 `ItemIndicator`와 같은 자리다). 같은 이유로 `MenubarCheckboxItem`과
 * `MenubarRadioItem`의 조합 스타일은 서로 같다 — 둘을 가르는 것은 역할과 표식이지 면이
 * 아니다. */
...
/* `MenubarPortal`은 공개하지 않는다 — 여기와 `MenubarSubContent`가 각자 Portal을 감싸므로
 * 소비처가 조립할 자리가 아니다(#172, ADR-0018). 포탈 대상을 고르는 경로가 필요해지면
 * 노드가 아니라 이 컴포넌트의 prop으로 온다. */
...
const componentContract = {
  ...
  /* 경계의 이웃: 진입점이 하나뿐인 행·캔버스 메뉴는 Dropdown Menu이고 우클릭으로 여는
