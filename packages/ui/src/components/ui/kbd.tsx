 ...
 *     따로 아는 것 둘 다 우리 규약 밖이다(button.tsx 머리말). 대신 그 수요를
 *     계약의 `limits`에 확인된 공백으로 적었다 — 침묵으로 두지 않는다.
 *
 ...
 *
 * 크기 축은 두지 않는다. cva를 그래도 쓰는 것은 규약이다 — 매니페스트의 출처가
 * cva 정의여야 하고, config를 이름 붙여 내보내야 축이 없다는 사실도 기계가 읽는다. */
...
/** 키 여러 개를 한 조합으로 묶는다. 구분자(`+`)는 소비처가 넣는 텍스트다. */
