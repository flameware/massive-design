 ...
 * 채널    붙은 모서리와 겹친 테두리를 얹는 `[&>*:not(:first-child)]` 계열은 셀에
 *         나타나지 않고 매니페스트의 `elsewhere`가 진다 — 그 모서리는 Figma에
 *         실재하되 이 컨테이너 자산이 아니라 **조립된 그룹**에 그려진다(ADR-0012).
 *         Toggle Group은 같은 일을 항목의 셀에서 하는데, 그쪽으로 옮길 자리가
 *         여기엔 없다 — 경계 ①이 자식의 props를 건드리지 않기로 했고 자식은 남의
 *         컴포넌트다. */
 ...
 * 한 글자도 바꾸지 않으므로 이 축은 **additive**다(#143의 `knockout: none`, #144의
 * `placement: auto`, #145의 `frame`과 같은 자리).
 *
 ...
 * 그래서 어긋난 짝(가로 그룹에 가로 선)을 계약이 막지 못한다.
 ...
 * `border-l-0`으로 접었고, 새 여백은 발행된 인스턴스를 움직여 additive가 아니게 된다. */
const buttonGroupSeparatorVariantsConfig = {
  variants: {
     ...
     * `vertical`은 가로 그룹 사이에 서는 세로 하이라인(기본값), `horizontal`은 세로
     * 그룹 사이에 서는 가로 하이라인이다. */
...
/* 겹친 테두리 대신 명시적 구분선이 필요할 때 쓴다. 항상 decorative다 — `Separator`의
 * 기본값이 `decorative`이므로 `role="none"`이 되어 `role="group"`인 그룹의 자식 목록에
 * 의미 노드를 더하지 않는다. 그룹이 뜻으로 갈린다면 그것은 구분선이 아니라 두 개의
 * `ButtonGroup`이다. */
...
const componentContract = {
  ...
  behaviors: {},
  reference: { example: "button-group", guidance: { use: "서로 무관한 동작 버튼을 한 덩어리로 붙여 보이는 자리에 쓴다. 자식마다 탭 정지와 각자의 `disabled`가 남는다.", evidence: "투자 이력의 행 도구 모음처럼 내보내기·인쇄·행 메뉴가 나란히 서야 하고, 그중 하나만 비활성이 되는 자리가 있다.", limits: "하나의 값을 고르는 선택 위젯에는 Toggle Group을 쓴다. `disabled`는 그룹이 아니라 각 Button에 준다. 구분선의 방향은 `ButtonGroupSeparator`의 `orientation`으로 그룹과 짝 맞춘다. 뜻으로 갈리는 묶음은 구분선이 아니라 두 `ButtonGroup`이다." } },
} as const
