...
const componentContract = {
  ...
  behaviors: {},
  reference: { example: "dialog", guidance: { use: "현재 흐름을 잠시 멈추고 집중해서 완료할 작업을 연다.", evidence: "투자 거래를 추가하거나 편집하는 동안 제목·설명·행동을 한 모달 맥락에 유지해야 한다.", limits: "파괴적 행동 확인에는 Alert Dialog를 사용하고, 단순 보조 정보에는 Popover를 사용한다. `DialogPortal`은 공개하지 않는다 — `DialogContent`가 스스로 Portal을 감싸므로 소비처가 조립할 자리가 아니다. 공개돼 있던 동안에도 쓰면 Portal이 이중으로 생겨 `container`가 무시됐다(#172, ADR-0018). 포탈 대상을 고르는 경로가 필요해지면 노드가 아니라 `DialogContent`의 prop으로 온다." } },
} as const
