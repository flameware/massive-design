...
const componentContract = {
  ...
  publicExports: ["Dialog", "DialogTrigger", "DialogPortal", "DialogClose", "DialogOverlay", "DialogContent", "DialogHeader", "DialogFooter", "DialogTitle", "DialogDescription", "dialogVariants", "dialogVariantsConfig"],
  config: dialogVariantsConfig, className: (props: Record<string, string>) => cn(dialogVariants(props)),
  anatomy: ["Dialog", "DialogTrigger", "DialogPortal", "DialogOverlay", "DialogContent", "DialogHeader?", "DialogTitle", "DialogDescription?", "DialogFooter?", "DialogClose?"],
  configurationStates: { open: ["closed", "open"] }, drawnBy: { open: "표면의 존재가 곧 열림이다 — 닫힌 상태에는 그릴 노드가 없다" },
  reference: { example: "dialog", guidance: { use: "현재 흐름을 잠시 멈추고 집중해서 완료할 작업을 연다.", evidence: "투자 거래를 추가하거나 편집하는 동안 제목·설명·행동을 한 모달 맥락에 유지해야 한다.", limits: "파괴적 행동 확인에는 Alert Dialog를 사용하고, 단순 보조 정보에는 Popover를 사용한다." } },
} as const

export { Dialog, DialogTrigger, DialogPortal, DialogClose, DialogOverlay, DialogContent, DialogHeader, DialogFooter, DialogTitle, DialogDescription, dialogVariants, dialogVariantsConfig, componentContract }
