 ...
 * 때문이고, 여기는 성립하지 않는다. #168이 보탠 두 번째 질문에도 걸린다: 원본이 지고
 * 있는 계약이 없는 표면을 소비하면 이 표면을 정의하는 유일한 선언이 `popover`
 * 매니페스트 밖에 남는다. 갈림선은 조합이냐 같은 역할이냐가 아니라 ⓐ 원본을 그대로
 * 렌더할 수 있는가와 ⓑ 원본이 질 계약이 있는가다 — 여기는 둘 다 아니라 복제가 남는
 * 유일한 길이고, 맨 클래스 문자열을 공유하는 선례가 카탈로그에 하나도 없는 것과도
 * 같다(#154). Dialog·Sheet·AlertDialog 셋이 이미 서로 복제한 자리다. 그리고 값도
 * 갈린다(아래).
 *
 ...
 * 유일한 설명 단인 `text-sm text-muted-foreground`다(Dialog·Sheet·AlertDialog·Card가
 * 같은 선언이고, 이 컴포넌트의 참조 시안이 이미 손으로 그 클래스를 적고 있었다) —
 * `/relaxed`는 어느 계약도 쓰지 않는 새 행간이고, 실측 수요 없이 단을 늘리지
 * 않는다(#168·#164가 값이 아니라 차이를 옮긴 것과 같다). 머리말은 `flex flex-col gap-1`이고
 * Dialog의 `text-center sm:text-left`를 가져오지 않는다 — 그 정렬은 작은 화면에서 폭을
 * 다 쓰는 모달의 결정이고 앵커에 붙는 288px 표면은 그런 상태가 없다. upstream 머리말의
 * `text-xs`도 가져오지 않는다: 두 자식이 각자 자기 단을 선언하므로 그것이 고르는 것은
 * 소비처가 머리말에 더 넣은 노드뿐이다. */
...
function PopoverContent({ className, align = "center", sideOffset = 4, ...props }: React.ComponentProps<typeof PopoverPrimitive.Content>) {
   ...
   * 그래서 `PopoverTitle`·`PopoverDescription`이 마운트된 동안에만 속성이 붙고, 없으면
   * 붙지 않는다. 표면의 접근 가능한 이름은 `PopoverTitle`이 지고 배선은 계약이 진다. */
  ...
  // 소비처가 스스로 이름을 준 표면은 건드리지 않는다. `aria-label`을 직접 주면 우리 `aria-labelledby`는
  // 서지 않고, `aria-labelledby`를 직접 주면 뒤의 전개가 이긴다 — 이름의 정본은 언제나 하나다
...
/* 머리말 셋(#166)은 새 토큰 0개이고 호환성은 additive — 기존 네 표면의 클래스·축·구성
 * 상태가 한 줄도 움직이지 않았고, 새 파트도 새 aria 속성도 소비처가 셋을 렌더하기 전에는
 * 나타나지 않는다. 여기서 새 비대칭이 하나 열린 채로 남는다: `dialog`·`sheet`·`alert-dialog`
 * 셋은 오늘도 `parts`가 없어 같은 역할의 Header·Title·Description이 어느 매니페스트에도
 * 닿은 적이 없다. 이제 popover의 셋만 파생 채널에 실리고 Dialog 쪽 셋은 실리지 않는다 —
 * 사실로 남기되 이 계약이 고칠 자리가 아니고 그 셋의 `parts`는 #155의 몫이다(#165 규칙 1:
 * 로스터를 손으로 넓히지 않는다). */
const componentContract = {
  name: "popover", source: "src/components/ui/popover.tsx",
  /* `PopoverPortal`은 공개하지 않는다 — `PopoverContent`가 각자 Portal을 감싸므로 소비처가
   * 조립할 자리가 아니다(#172, ADR-0018). 포탈 대상을 고르는 경로가 필요해지면 노드가 아니라
   * `PopoverContent`의 prop으로 온다. */
  ...
  /* 머리말 셋은 소비처가 조립하므로 anatomy다(#166, ADR-0018). `PopoverPortal`과 갈리는 자리가
   * 여기다 — Portal은 `PopoverContent`가 자동으로 감싸 소비처가 쓸 자리가 없지만, 셋은
   * 소비처가 `PopoverContent` 안에 직접 넣어야만 존재한다. 셋 다 선택적이고 서로를 요구하지
   * 않는다: `PopoverHeader`는 제목과 설명을 세로로 묶는 그릇일 뿐이라 한 조각만 쓰는 표면은
   * 그릇 없이 그 조각만 쓴다. 그래서 표기는 셋 다 `?`이고, 필수로 올리면 오늘 발행된
   * 인스턴스가 계약 위반이 된다. */
  ...
  /* `parts`를 이 세대가 신설했다(#166, #155 모집단 9 → 8). 새로 등록된 셋의 클래스에서 #154가
   * `DropdownMenuSeparator`에서 잡은 계열 위반(`--ds-border-default`가 `background-color`에
   * 오는 자리)은 나오지 않았다 — 셋 다 타이포그래피와 레이아웃이고 색은 `text-muted-foreground`
   * 하나뿐이라 전경 이름이 전경 속성에 온다. 루트의 클래스는 `parts` 없이도 이미 매니페스트에
   * 닿아 있었으므로(`config`·`className`이 루트 셀을 낸다) 이 신설로 처음 드러난 기존 셀은 없다. */
  parts: {
  ...
  /* hover 모드에서도 컨트롤의 의미를 보충하는 한 줄 설명이 여전히 Tooltip인 이유: Tooltip은
   * 트리거에 aria-describedby로 묶여 이름을 보조하는 설명이고, hover 모드의 Popover는 트리거가
   * 가리키는 대상의 미리보기다. 미리보기 안의 정보와 행동은 hover 없이도 도달할 수 있는 다른
   * 경로가 있어야 하며 이 모드는 필수 작업 흐름을 담지 않는다. 여는 지연과 닫는 지연은 우리가
   * 정하지만 공개 prop이 아니다. 그 표면들은 대개 "무엇에 대한 설정인가" 한 줄과 그 아래
   * 설명 한 줄로 시작한다 — 머리말 셋을 연 소비처 근거다. */
  reference: { example: "popover", guidance: { use: "트리거 가까이에 짧은 보조 정보·설정을 연다. `PopoverTitle`·`PopoverDescription`이 제목·설명을 지고, 클릭(기본)과 openOn=\"hover\"를 한 계약으로 덮는다.", evidence: "투자 기록의 필터 설명과 빠른 설정을 화면 맥락을 떠나지 않고 보여줘야 하고, 종목 이름 위에 머무르면 요약을 미리 보는 경로도 같은 자산이어야 한다. 그 표면은 대개 제목 한 줄과 설명 한 줄로 시작한다.", limits: "핵심 흐름·긴 양식은 Dialog, 행동 없는 짧은 설명은 Tooltip을 쓴다. hover Popover는 대상의 미리보기라 hover 없는 경로도 두고 필수 흐름을 담지 않는다. `PopoverPortal`은 공개하지 않고 포탈 대상은 `PopoverContent`의 prop으로 온다 — 근거: ADR-0018" } },
} as const
