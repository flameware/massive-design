...
/* `size`의 `xs` 값은 열지 않는다 — 실측 수요가 없기 때문이고(#123), `Card`의 근거로
 * 닫는 것이 아니다. `Card`·`AlertDialog`가 `size`를 닫은 문장(*축은 늘지만 간격은
 * 소비처가 유틸리티로 정하면 된다*)은 둘 다 **축이 아예 없는** 자리라 성립했다. 여기는
 * 축이 이미 있고 우리가 `default`·`sm` 두 단의 간격을 정해 발행했으므로, 소비처는
 * 유틸리티를 **더하는** 것이 아니라 계약이 낸 `gap-4 px-4 py-3`을 **덮어써야** 하고
 * 그렇게 덮어쓴 값은 매니페스트에 닿지 않는다 — 파생 채널에 없는 크기가 코드에
 * 돌아다니는 모양이라 침묵보다 한 걸음 나쁘다(ADR-0006). **그래서 이 자리는 수요가
 * 확인되면 여는 것이 기본값이다**(#174). 그때 비용은 축 신설이 아니라 루트 셀 6 → 9이고
 * 파트 아홉은 `size`를 상속하지 않아 늘지 않는다. 값 이름은 upstream 그대로 `xs`이며
 * `Button`이 이미 쓰고 있어 어휘가 갈리지 않는다 — 값 이름은 축 지역이므로 충돌 자체가
 * 물을 것이 아니다(ADR-0008). */
 ...
 * `frame`은 갈리는 셋(지름·면·자르기)을 한 낱말로 부른다. 값 이름은 upstream을 그대로
 * 쓴다(#121) — `none`(틀 없음, 기본값)·`icon`(글리프용 `size-8` 면)·`image`(그림용
 * `size-10` 자르기 틀) 셋이다.
 *
 * **아바타는 `frame`의 값이 아니다** — upstream에도 없고, 원형 틀·지름·겹침 링은 우리
 * `Avatar`가 이미 지는 결정이라 값으로 열면 그 결정을 복제한다(#91).
 * `<ItemMedia frame="none"><Avatar/></ItemMedia>`로 **소비한다**: `Avatar`가 자기 틀을
 * 그리므로 `image` 안에 넣으면 틀이 겹친다.
 *
 ...
 * **계약을 여는** 방향이라 #140의 destination과 반대다(ADR-0013). 틀 자체의 결정은
 * 전부 해결된 속성으로 떨어지므로 축이 침묵하지는 않는다.
 *
 * 대체 텍스트는 계약이 지지 않는다 — 슬롯은 자기가 담은 것이 장식인지 알 수 없다.
 * 장식이면 `ItemMedia`에 `aria-hidden`을 걸고, 뜻이 있으면 소비처가 안쪽 `<img>`의
 * `alt`에 넣는다. */
...
const componentContract = {
  ...
  behaviors: {},
  reference: { example: "item", guidance: { use: "미디어·주 정보·보조 설명·행동을 재배치 가능한 한 항목으로 조립할 때 쓴다. 미디어 자리의 틀은 `ItemMedia`의 `frame`으로 고른다.", evidence: "검색 결과, 선택 목록, 설정 행처럼 같은 정보 위계를 공유하지만 제품 의미가 다른 반복 항목이 필요하고, 같은 목록 안에서 통화 기호 같은 글리프와 종목 로고 이미지가 같은 자리에 번갈아 선다.", limits: "탐색·선택·버튼 역할은 주지 않는다 — 상호작용 의미는 소비처가 명시한다. 아바타는 `frame=\"none\"` 안에 `Avatar`를 넣는다(`image`는 틀이 겹친다). 대체 텍스트는 장식이면 `ItemMedia`에 `aria-hidden`, 뜻이 있으면 `<img>`의 `alt`로 소비처가 준다." } },
} as const
