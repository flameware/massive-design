 ...
 *
 * **밑줄 형태에서도 목록은 가로로 늘어선다.** `orientation: vertical`이 옮기는 것은
 * 목록과 패널의 관계이지 목록 안의 방향이 아니므로 기준선은 두 축 모두에서 목록의
 * 아래 변이다. 목록의 폭은 계약하지 않는다(`w-fit`이 두 형태에 같다) — 기준선을 본문
 * 폭까지 늘리는 것은 소비처가 `className`으로 정한다. */
...
const componentContract = {
  ...
  /* "URL로 직접 접근해야 하는 화면 탐색에는 링크나 내비게이션을 사용한다"가 가리키던
   * 자리가 Navigation Menu로 카탈로그에 실제로 생겨 경계가 닫혔다(#127): 주소가 바뀌면
   * Navigation Menu, 같은 화면 안에서 패널만 갈아 끼우면 Tabs다. */
  reference: { example: "tabs", guidance: { use: "같은 화면 안에서 콘텐츠 패널을 하나씩 갈아 끼울 때 쓴다. 활성 탭을 알약으로 표시할지 밑줄로 할지 `TabsList`의 `indicator`로 고른다.", evidence: "투자 상세에서 보유 현황과 거래 내역처럼 동일 대상의 병렬 보기를 화면 이동 없이 전환해야 하고, 본문 위에 얹히는 탭 막대는 트랙 없이 기준선 하나로 서야 한다.", limits: "주소가 바뀌는 화면 탐색은 Navigation Menu다 — 여기는 같은 화면 안에서 패널만 갈아 끼운다. 밑줄 형태에서도 목록은 가로로 늘어선다(`orientation: vertical`은 목록과 패널의 관계만 옮긴다). 목록의 폭은 계약하지 않는다 — 기준선을 본문 폭까지 늘리는 것은 소비처가 `className`으로 정한다." } },
} as const
