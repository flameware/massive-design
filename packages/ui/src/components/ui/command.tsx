...
/* `CommandSeparator`는 43세대 동안 이름조차 없던 종류 ② 공백이었고 #169가 ADR-0006의
 * 두 관문으로 판정해 열었다(#165) — upstream에 실재하는 노드이고(ⓐ), 소비처가 다시
 * 그리려면 목록 안쪽 여백을 되쓰는 음수 여백을 우리 내부 수에서 추측해야 한다(ⓑ).
 * 새 축도 새 구성 상태도 늘지 않는다 — 정적 파트 하나다.
 *
 ...
 * 같은 모양이라 베끼면 43세대짜리 결함을 새로 만든다. 렌더는 같은 1px 선이고
 * Menubar·Resizable이 이미 낸 답을 같은 이유로 여기서도 쓴다.
 *
 ...
 * 소비처는 볼 수도 없는 우리 내부 수이고, 소비처에 맡기면 목록 패딩을 한 번 바꿀 때마다
 * 모든 소비처에서 선이 어긋난다 — 두 수는 한 파일에서 함께 움직여야 하고, 파생 채널이
 * 나르는 기하도 계약이 질 때만 참이다.
 ...
 *
 * 검색으로 한쪽 묶음이 비어도 선은 그대로 남는다 — `CommandGroup`이 자기 묶음의 일치 수를
 * 세지 않기 때문이고, 오늘 빈 묶음의 `CommandGroupHeading`이 남는 것과 같은 자리다.
 * 여는 티켓(#169)이 그 계산을 새로 들이지 않았다. */
...
const componentContract = {
  ...
  /* 항목 끝의 단축키 표기(upstream의 `CommandShortcut`)는 열지 않는다 — 소비처가 `Kbd`를
   * `ml-auto`와 함께 놓으면 되고 그 클래스가 우리 스타일 결정을 복제하지 않는다(#121 ⓑ,
   * `InputGroupButton`의 variant·size와 같은 판정 2). upstream의 `CommandShortcut`도 키캡이
   * 아니라 평평한 muted 텍스트라 Kbd가 채우던 자리가 아니다. */
  ...
  /* `CommandSeparator?`는 존재와 선택성만 말한다 — 이 선이 가르는 것은 `CommandList`의 형제
   * 구역이고 `CommandGroup` 사이 전용이 아니라, 묶음이 없는 목록에서 항목 무리를 가르는 쓰임도
   * 같은 자리다. 쓰임이 한 자리로 고정되지 않으므로 anatomy는 자리를 말하지 않는다(anatomy
   * 표기는 평평한 목록이라 중첩을 말하지 못한다). 고정된 것은 가면 안 되는 자리다: `CommandGroup`
   * 안에는 넣지 않는다 — 묶음은 `aria-labelledby`로 이름 하나를 갖는 한 구역이라 그 안을 선으로
   * 가르면 이름이 가리키는 범위와 눈에 보이는 범위가 어긋난다. */
  ...
  behaviors: {},
  reference: { example: "command", guidance: { use: "검색어로 목록을 좁혀 명령이나 항목 하나를 고르고, 키보드 커서(highlighted)와 고른 값(selected)을 함께 보여준다.", evidence: "종목·거래·화면 이동이 한 자리에 섞여 있어 마우스로 메뉴를 파고들기보다 이름을 입력해 바로 실행하는 진입점이 필요하다.", limits: "값이 적고 고정된 선택에는 Select, 맥락 동작 묶음에는 Dropdown Menu를 쓴다. 팝오버·모달 안에 넣기와 닫기, 원격 검색과 정렬, 검색 입력의 접근 가능한 이름은 소비처가 준다. 단축키 표기는 `Kbd`를 `ml-auto`로 놓는다. `CommandSeparator`는 `CommandGroup` 안에 넣지 않는다." } },
} as const
