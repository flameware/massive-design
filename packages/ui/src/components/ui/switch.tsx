...
const THUMB = "pointer-events-none block translate-x-0 rounded-full bg-background ring-0 transition-transform"
...
function Switch({ className, size = "default", ...props }: React.ComponentProps<typeof SwitchPrimitive.Root> & VariantProps<typeof switchVariants>) {
  return <SwitchPrimitive.Root data-slot="switch" data-size={size} className={cn(switchVariants({ size, className }))} {...props}><SwitchPrimitive.Thumb data-slot="switch-thumb" className={THUMB} /></SwitchPrimitive.Root>
}
