...
const componentContract = {
  ...
  reference: { example: "avatar", guidance: { use: "사람이나 계정을 작은 원형 이미지로 식별하고 이미지가 없거나 실패하면 안정적인 fallback을 표시한다.", evidence: "투자 기록의 작성자나 연결된 증권 계정을 목록과 활동 내역에서 빠르게 구별해야 한다.", limits: "이미지만으로 이름을 전달하지 말고 주변 텍스트나 접근 가능한 이름을 제공하며, 장식 이미지에는 빈 대체 텍스트를 사용한다. upstream의 `AvatarBadge`·`AvatarGroup`·`AvatarGroupCount`는 계약하지 않는다 — anatomy가 늘고 겹침·ring 배치를 소비처가 복제해야 하는 진짜 표면이라 열 근거는 있으나, 기존 카탈로그를 다시 여는 일이라 별도 effort에서 additive로 붙인다(#121)." } },
} as const
