 ...
 * **트랙의 형제**이고, 둘을 담는 행은 소비처가 준다. 텍스트가 들어가도록 루트의 격자를
 * 바꾸는 것은 발행된 인스턴스의 재해석이라 `breaking`이다. 그래서 둘을 담는 한 줄(`flex
 * items-center gap-2` 정도)은 소비처가 주고 계약은 **두 조각의 타이포그래피와 접근성만**
 * 진다. `VALUE`의 `ml-auto`는 upstream에서 그대로 가져온 것으로 그 행이 flex·grid일 때
 * 값을 오른쪽 끝으로 민다 — 그런 행이 아니면 아무 일도 하지 않는다. `ProgressLabel`은
 * `<span>`이라 자기 힘으로 이름을 붙이지 못한다: 소비처가 `id`를 주고 `Progress`에
 * `aria-labelledby`로 물린다(`aria-label`을 루트에 직접 주는 길도 그대로 열려 있다).
 *
 * 새 토큰 0개다. `tabular-nums`는 Tailwind의 `font-variant-numeric` 유틸리티이지
 * 토큰 수요가 아니고, Calendar가 이미 같은 유틸리티를 쓴다 — 숫자 폭을 토큰으로 올리려면
 * 타이포 스케일 전체의 결정이 먼저다(#165 규칙 5). 그 유틸리티가 매니페스트에서 `--tw-*`
 * 합성 사슬을 그대로 흘리는 것은 이 계약의 결함이 아니라 #140이 남긴 열린 안개와 같은
 ...
 * 뒤에 온다(`aria-hidden={undefined}`). 이 파트는 자기 수를 만들지 않고 children으로
 * 받는다 — 루트에 값 context가 없고, 그것을 여는 것은 새 표면이라 #165의 로스터 밖이다. */
...
/* #167 세대의 호환성 분류는 `additive`다 — 파트 셋과 공개 export 둘이 늘 뿐 기존 셀은
 * 그대로다. `ProgressIndicator` 등록만 따로 보면 `in-place safe`다. */
const componentContract = {
  ...
  /* `ProgressIndicator`는 오늘도 그리던 노드에 이름이 붙은 것이라 렌더가 한 픽셀도 움직이지
   * 않는다(`in-place safe`). `parts`를 처음 세우면서 그 클래스가 매니페스트에 처음 닿았고, #154가
   * `DropdownMenuSeparator`에서 잡은 계열 위반(`--ds-border-default`를 `background-color`에) 같은
   * 것은 나오지 않았다 — `bg-primary`는 배경 계열 이름이 배경 속성에 온 자리다. */
  parts: {
