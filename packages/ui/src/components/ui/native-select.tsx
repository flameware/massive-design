...
/* `NativeSelectGroup`은 upstream의 `NativeSelectOptGroup`과 이름이 다르고, 그대로 둔다 —
 * 바꿀 수 없어서가 아니라 바꾸지 않는 것이 옳아서다(#173). 카탈로그의 그룹 노드는 스무
 * 개가 전부 `*Group`이고(`SelectGroup`·`CommandGroup`·`FieldGroup`·`AvatarGroup`…), upstream의
 * `OptGroup`은 HTML 요소 이름 `<optgroup>`을 따른 것이다. 하나만 HTML 이름을 따르게 하면
 * 노드 이름의 전역 일관성이 깨진다(ADR-0008 규칙 1의 정신). `breaking`이라 못 바꾼다는
 * 것은 근거가 아니다 — ADR-0018이 미발행·소비처 부재 동안 `breaking` 실행이 열려 있음을
 * 정했고 리포 안 사용처도 참조 시안 하나뿐이다. */
...
const componentContract = {
  ...
  behaviors: {},
  reference: { example: "native-select", guidance: {
