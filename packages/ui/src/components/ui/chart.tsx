 ...
 *
 * upstream이 이 주입 통로에 붙인 이름도 `ChartStyle`이고 이 파일은 **같은 이름의 같은
 * 노드**를 갖는다 — 다만 export하지 않는다(#162가 종류 ②로 찾았고 #175가 판정했다).
 * `<style>` 한 덩이를 내는 노드라 클래스도 축도 구성 상태도 없어 anatomy에 이름을 얻을 수
 * 없고(대문자 공개 export는 anatomy에 있어야 한다, ADR-0006), `ChartContainer`가 그것을
 * 렌더하므로 `--color-<key>` 통로는 이미 이 계약 안에 있다. upstream의 `ChartStyle`이
 * `THEMES` 두 벌을 내고 우리 것이 한 벌인 차이는 위 다크 모드 판정이 진다. */
...
const componentContract = {
  ...
  configurationStates: {},
  behaviors: {
    pointerReveal: { kind: "open-cause", surface: "ChartTooltipContent", origin: "inherited", why: "recharts `Tooltip`의 `trigger` 기본값이 `'hover'`이고 소비처가 그것을 통과시킨다 — **우리 툴팁 카드는 포인터가 차트 본체 위에 있을 때만 나타난다.** 카드는 우리 것이고 anatomy에 있지만 나타나는 계기는 우리 노드 밖에 있다: `externalSurfaces`가 이미 래퍼의 위치·표시 여부를 recharts 것으로 적었고(#122) 여기 적는 것은 그 표시가 **무엇에 걸려 있는가**다. 생성된 스토리는 카드를 정적으로 렌더하므로 실제 계기를 한 번도 밟지 않고, 포인터가 없는 사용자에게는 recharts의 `accessibilityLayer`가 여는 키보드 경로만 남는다 — 그 층도 우리 계약에 노드가 없다. 소비처가 `Tooltip`의 `trigger`로 바꾼다(#187)." },
  },
  /* 데이터 표를 대신하지 않는다 — 값 자체를 읽어야 하는 자리에는 Table을 쓴다. */
  reference: { example: "chart", guidance: { use: "시계열이나 범주 비교를 Recharts로 그리면서, 툴팁 카드와 범례만 카탈로그의 면·글자·모서리 규칙에 맞춰 통일한다.", evidence: "투자 이력은 월별 매수·매도 금액과 평가금액 추이를 같은 화면에서 보여주고, 그 위에 뜨는 툴팁과 범례가 카드·팝오버와 다른 면으로 보이면 같은 앱으로 읽히지 않는다.", limits: "차트 본체는 Recharts prop(`stroke`·`tick`·`fill`)으로 소비처가 그린다. `Tooltip`·`Legend`는 recharts에서 가져와 `content`에 카드를 꽂는다. 계열 색은 `ChartConfig.color`로 모드 전환이 끝난 변수를 준다. 값을 읽는 자리는 Table이다." } },
} as const
