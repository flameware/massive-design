 ...
 * 둘 다 나중에 여는 것은 additive이고 지금 닫는 근거는 여기다.
 *
 * 위 Viewport 판정은 Radix 이름으로 적혀 있다 — upstream은 Base UI 갈래로 옮겨 가면서
 * 같은 자리를 `NavigationMenuPositioner`라는 이름으로 공개하고, 그 하나가 `Portal` ›
 * `Positioner` › `Popup` › `Viewport` 넷을 묶어 치수를 `--positioner-height`·
 * `--positioner-width`로 **실행 중 측정**해서 받는다. 노드가 늘어난 것이 아니라 이름이
 * 갈린 것이고 측정으로 자기 치수를 얻는 성질도 그대로라 판정은 같다 — 이름이 달라 다음
 * 재조회가 이것을 새 표면으로 잡지 않도록 여기 적는다(#162가 찾았고 #175가 판정했다).
 * primitive 갈래 자체는 ADR-0016이 `radix-ui`로 고정했으므로 이 이름을 선취하지 않는다.
 *
 ...
 * 아니라 파생 채널에 자리가 없어 `limits` 산문이 값을 낸다 — 라우터 배선과 활성 판정은
 * 소비처가 소유한다.
 *
 * ── 접근성 ──────────────────────────────────────────────────────────────────
...
const componentContract = {
  ...
  /* Tabs의 limits가 "URL로 직접 접근해야 하는 화면 탐색에는 링크나 내비게이션을
   * 사용한다"고 가리킨 자리가 여기다(#127) — 같은 화면 안에서 패널만 갈아 끼우는 전환은
   * Tabs이고, 주소가 바뀌는 이동은 이쪽이다. 진입점 하나에 달린 보조 동작 묶음은
   * Dropdown Menu다. */
  reference: { example: "navigation-menu", guidance: { use: "화면 상단에서 사이트의 주요 목적지를 가로로 늘어놓는 자리에 쓴다. 하위 목적지가 여럿인 항목만 카드로 펼치고, 현재 화면 링크는 `active`로 표시한다.", evidence: "투자 기록은 포트폴리오·거래·회고가 각각 URL로 직접 열려야 하는 별개 화면이고, 포트폴리오 아래에는 보유 현황·비중 같은 하위 목적지가 더 있어 막대에서 한 겹 펼쳐 보여야 한다.", limits: "패널 전환은 Tabs, 명령 메뉴는 Menubar, 보조 동작은 Dropdown Menu다. 하위 없는 항목은 `NavigationMenuLink`에 `navigationMenuTriggerVariants()`를 입혀 막대에 놓고 라우터 링크는 `asChild`로 끼운다. `aria-label`과 라우팅은 소비처가 준다." } },
} as const
