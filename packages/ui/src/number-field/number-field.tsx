 ...
 * 8% 층을 쓴다. base는 **Group과 같은 `bg.surface`**다(#463): 한 컨트롤 안에서
 * 면색이 둘로 갈리면 −·값·+ 세 칸이 한 덩어리로 읽히지 않는다. 회색
 * (`neutral-soft`)으로 두던 근거는 "흐름 안의 버튼이라 떠 있는 팝업의 항목
 * (SelectItem·MenuItem이 #387에서 `bg-overlay`로 옮겼다)과 같은 자리가 아니다"
 * 였고, 그 판단은 그대로다 — 옮겨간 곳이 팝업 면이 아니라 자기 컨트롤의 면일
 * 뿐이다. 어포던스는 면색이 아니라 `.state`의 hover·active 층이 진다. Base UI가
...
const numberFieldStepperVariants = cva([
  ...
  "[--ds-state-base:var(--ds-bg-surface)]",
  // 무력화에서도 한 면이어야 한다 — `.state`는 alpha 0%에서 기준색을 그대로
  // 칠하는 background-color의 유일한 작성자라(state.css), 기준색을 그대로 두면
  // Group이 `data-disabled:bg-inset`으로 갈 때 ± 칸만 흰 면으로 남는다. 쉬는
  // 면을 한 덩어리로 만든 것과 같은 이유로 여기도 따라간다(#463).
  "data-disabled:[--ds-state-base:var(--ds-bg-inset)]",
