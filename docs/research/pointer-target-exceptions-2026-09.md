...
실측 미달 15항목(라벨·트랙 포함) 중 **예외로 확정되는 것은 5항목**이다 — 구조(`overflow` 조상에 잘림) 2, 외부 소유 1, WCAG 2.5.8 예외 조항(Equivalent) 2(라벨·Slider track, 둘 다 [#230](https://github.com/flameware/massive-design/issues/230) 착지로 조건이 충족돼 [#249](https://github.com/flameware/massive-design/issues/249)가 확정했다). 나머지는 [#230](https://github.com/flameware/massive-design/issues/230)이 규칙을 적용하거나(9항목), 이 map 밖의 별도 결함(Sidebar 아이콘 모드 미구현, 4항목·§5)으로 갈린다. 원래 이슈 본문이 적은 "세 종류"(overflow 조상/`::after` 점유/외부 소유) 중 **`::after` 점유는 실측으로 폐기된다** — 그 자리라고 지목됐던 Resizable은 자기 `after:`가 이미 원하는 히트 영역을 주고 있어 충돌이 없고, 실제 벽은 외부 소유다(§2.2).
...
### 4.1 라벨(`label`·`field-label`) — WCAG 예외 조항

> **확정 (2026-09-04, [#249](https://github.com/flameware/massive-design/issues/249)).** [#230](https://github.com/flameware/massive-design/issues/230)이 병합돼(main `76b38b4`) Checkbox·Radio·Switch가 24×24에 닿았다 — 재실측(`docs/research/pointer-targets-2026-09.tsv`)이 셋 다 `square24: true`를 확인한다. 아래 조건부 문단이 요구한 전제가 완성됐으므로 이 예외는 더 이상 조건부가 아니다.
...
**Equivalent 예외가 성립한다.** 라벨의 유일한 기능은 연결된 컨트롤을 활성화(토글/포커스)하는 것이고, **같은 기능을 컨트롤 자신을 눌러도 얻는다.** 페어링 대상별로:

| 페어링 컨트롤 | 24×24 충족 | 근거 |
|---|---|---|
| Checkbox·Radio·Switch | 충족(#230 착지 확인) | #230이 24×24로 올렸다 — 재실측 확인(#249) |
| Input·Textarea·Native Select 등 | 지금 이미 충족 | 이 컨트롤들은 미달 목록(§2·연구 문서 §4.1)에 없다 |

`FieldLabel`은 어느 컨트롤과도 짝지어질 수 있는 범용 파트라, 예외가 **컨트롤에 의존한다**는 조건을 명시한다. #230이 Checkbox·Radio·Switch를 24×24로 올려 전제가 완성됐다(위 확정 문단) — 라벨 자체를 뚫는 별도 작업은 필요 없었다: 짝 컨트롤이 뚫리자 라벨이 자동으로 예외 조건을 만족했다.

### 4.2 Slider track(`slider-track`) — WCAG 예외 조항

> **확정 (2026-09-04, [#249](https://github.com/flameware/massive-design/issues/249)).** [#230](https://github.com/flameware/massive-design/issues/230)이 병합돼 Slider thumb(sm/default/lg)가 24×24에 닿았다 — 재실측이 셋 다 `square24: true`를 확인한다. 경로 1(Thumb 드래그)의 전제가 완성됐으므로 이 예외도 더 이상 조건부가 아니다.
...
**Equivalent 예외가 성립한다 — 이중 경로.** Track이 주는 기능(값 설정)은 이미 두 개의 다른 경로로 동등하게 제공된다.
...
| 15 | Label/FieldLabel | `label`·`field-label` | **예외 — WCAG(Equivalent)** | §4.1 |
| 16 | Slider | `slider-track` | **예외 — WCAG(Equivalent)** | §4.2 |
...
- 라벨·Slider track의 예외는 [#230](https://github.com/flameware/massive-design/issues/230) 착지에 조건부였다 — Checkbox·Radio·Switch·Slider thumb가 24×24에 닿지 못하면 Equivalent 전제가 무너진다. [#249](https://github.com/flameware/massive-design/issues/249)가 #230 착지 뒤 재실측으로 §4를 재확인했고, 넷 다 `square24: true`라 두 예외 모두 확정됐다(§4.1·§4.2).
