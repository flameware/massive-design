...
**그래서 모집단은 기계가 만든다** — 게이트가 못 보는 자리라고 해서 재조회 전체가 사람의 주의력에 걸려야 하는 것은 아니다. [#121](https://github.com/flameware/massive-design/issues/121)의 구멍은 우연이 아니라 규칙적이었고([#162](https://github.com/flameware/massive-design/issues/162) §3), 원인은 **비교의 단위가 식별자가 아니라 표의 행이었다**는 것이다. 그러므로 모집단은 upstream 레지스트리 소스와 우리 `publicExports`·매니페스트 `axes`의 **기계적 집합 차집합**으로 만들고, 관문 ⓐ·ⓑ는 그 목록이 완결된 **뒤에** 건다. 절차와 재현 명령은 [`docs/agents/upstream-surface-recount.md`](../agents/upstream-surface-recount.md)에 있다. **이것은 게이트가 아니다** — 네트워크와 남의 리포를 읽어야 하므로 CI가 지킬 수 없고, 위 문단의 한계는 그대로 선다. 바뀌는 것은 재조회의 **모집단**이 사람의 독해에 의존하지 않게 된다는 것뿐이다([#176](https://github.com/flameware/massive-design/issues/176)).
