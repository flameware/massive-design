...
*[#334](https://github.com/flameware/massive-design/issues/334)이 이 절의 경계를 한 번 확인했다.* 차트 본체·라이브러리는 그대로 소비처 것이고 이 ADR은 뒤집히지 않는다 — 다만 소비처가 **브랜드에서 계열색을 뽑는 규칙**은 DS가 문서로 준다([`packages/tokens/README.md` §차트 계열색](../../packages/tokens/README.md)). 옮긴 것은 차트이지 브랜드 파생이 아니기 때문이다. 토큰도 컴포넌트도 늘지 않았다.
