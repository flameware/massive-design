...
⚠️ **발행되어 최신인 상태의 이름은 `CURRENT`다. `PUBLISHED`가 아니다** ([#273](https://github.com/flameware/massive-design/issues/273)). 이 표는 두 세대 동안 `PUBLISHED`로 적혀 있었다 — 런타임이 내지 않는 값이다. 표를 곧이곧대로 구현하면 **모든 자산이 영원히 미발행으로 읽히고**, 사람은 누를 것이 없는 발행 버튼을 매 세대 요구받는다. 조용해야 할 값이 무엇인지가 이 표의 전부이므로 틀린 이름 하나가 표 전체를 뒤집는다.
