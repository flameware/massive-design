...
**무엇을 돈다.** `bun run pointer-gate`(`scripts/pointer-target-gate.mjs`, [#232](https://github.com/flameware/massive-design/issues/232))를 돈다 — 스캔([`pointer-target-measure.md`](pointer-target-measure.md) §2.1)과 렌더 실측(§2.2) 위에 서서 기준선([`pointer-targets-2026-09.md`](../research/pointer-targets-2026-09.md)의 TSV)과 diff하는 게이트다. **CI 승격은 보류한다**(ADR-0020 결정 6, `bun run check`·preflight 사슬에 없다 — Storybook 빌드 + Playwright가 약 7분) — 지금은 손으로 돈다. 게이트가 재는 것은 인상이 아니라 항목이다: 스캔 slot 각각이 24×24 하한([ADR-0020](../adr/0020-pointer-target-size-is-borne-by-the-hit-area.md) 결정 1)을 만족하는가(만족 못 하면 예외 목록 [`pointer-target-exceptions-2026-09.md`](../research/pointer-target-exceptions-2026-09.md)와 그 게이트용 사본 `docs/research/pointer-target-exceptions.json`에 이름과 이유로 올라 있는가), 히트 영역의 오버리지가 기준선에 이미 선언돼 있는가. 게이트 출력은 기준선 대비 새 미달(regression)과 기준선에도 있던 미달(known gap)을 갈라 보여준다 — #230·#249가 끝나기 전까지는 known gap이 남아 게이트가 빨갛다, 이것은 CI에 물리지 않았으므로 병합을 막지 않는다.
