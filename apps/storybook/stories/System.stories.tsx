...
export const FreshnessAndCoverage: Story = { render: () => {
  ...
  return <main className="mx-auto grid max-w-6xl gap-6 p-8"><header><p className="text-sm text-muted-foreground">Repo verification · explicit Figma Sync</p><h1 className="text-3xl font-semibold">Is the design system current?</h1><p>Repo verification은 Storybook에서 독립적으로 완료된다. Figma 기준선은 사용자가 Sync를 요청할 때만 전진한다.</p></header>
    ...
    <section className="grid gap-3"><div><h2 className="text-xl font-semibold">Figma public baseline</h2><p className="text-sm text-muted-foreground">{sameGeneration ? "Same generation" : `Different generation · allowed until explicit Figma Sync · token ${tokenChanged ? "changed" : "same"} · ${changedComponents.length} component(s) changed`}</p></div><StageCards names={figmaStages} record={figmaBaseline} /></section>
    <p className="text-sm">Repo inputDigest: <code>{repoRecord?.inputDigest ?? "record unavailable"}</code> · Figma inputDigest: <code>{figmaBaseline?.inputDigest ?? "baseline unavailable"}</code> · resumeAt: <strong>{repoRecord?.resumeAt ?? "none"}</strong></p>
