...
export const UpdateRunbook: Story = { render: () => <main className="mx-auto grid max-w-3xl gap-5 p-8"><h1 className="text-3xl font-semibold">Update runbook</h1><ol className="list-decimal space-y-3 pl-6"><li>코드 정본을 이름으로 찾아 제자리에서 변경한다.</li><li><code>bun run sync:preflight</code>로 token·manifest·Storybook 생성과 check·test·build·axe를 실행한다.</li><li>검증 기록의 inputDigest와 <code>CODE_VERIFIED</code>, <code>STORYBOOK_VERIFIED</code> 증거를 확인한다.</li><li>Figma 문서에서 같은 이름의 자산에 token과 component를 제자리 주입하고 멱등 재실행을 확인한다.</li><li>변경된 세대를 사람이 시각 검토하고 라이브러리를 발행한다. Variables·Text Style·Effect Style의 미발행 변경 없음도 사람이 확인한다.</li><li>모든 대상의 <code>CURRENT</code>와 소비 파일 구독·원격 import를 확인한 뒤에만 완료로 판정한다.</li></ol><p>중단하면 검증 기록의 <code>resumeAt</code>에서 재개한다. 사람 발행 대기는 실패가 아니라 <code>PENDING_HUMAN</code>이다.</p></main> }
