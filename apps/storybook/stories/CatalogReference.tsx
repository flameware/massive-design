...
function Preview({ name, selection = {} }: { name: CatalogEntry["reference"]["example"]; selection?: Record<string, string> }) {
  ...
  const previews: Record<CatalogEntry["reference"]["example"], ReactNode> = {
    ...
    select: <Select defaultOpen={selection.open === "open"}><SelectTrigger aria-label="시장 필터" size={size} className="max-w-xs"><SelectValue placeholder="시장 선택"/></SelectTrigger><SelectContent><SelectItem value="kr">국내</SelectItem><SelectItem value="us">미국</SelectItem></SelectContent></Select>,
...
export function CatalogReference({ entry, selection = {} }: { entry: CatalogEntry; selection?: Record<string, string> }) {
  ...
  return <main className="mx-auto grid max-w-5xl gap-6 p-4">
    <header><p className="text-sm text-muted-foreground">생성됨 · 직접 고치지 말 것 · 매니페스트 {entry.hash}</p><h1 className="text-3xl font-semibold">{entry.displayName}</h1><p>조합 {entry.cells}개 · 소스 파일 <code>{entry.source}</code></p></header>
    <section className="rounded-lg border bg-card p-5"><h2 className="mb-4 text-xl font-semibold">실행 예시</h2><Preview name={entry.reference.example} selection={selection}/></section>
    <section className="rounded-lg border p-5"><h2 className="text-xl font-semibold">생성된 계약</h2><p><strong>구성 요소:</strong> {entry.anatomy.join(" · ")}</p><p><strong>축:</strong> {Object.entries(entry.axes).map(([axis, values]) => `${axis}: ${values.join(" | ")}`).join("; ") || "없음"}</p><p><strong>구성 상태:</strong> {Object.entries(entry.configurationStates).map(([axis, values]) => `${axis}: ${values.join(" | ")}`).join("; ") || "없음"}</p><p><strong>확인해야 하는 화면:</strong> Light · Dark{entry.stateSamples ? " · hover · pressed · disabled" : ""}</p></section>
    <section className="rounded-lg border p-5"><h2 className="text-xl font-semibold">사용 지침</h2><p><strong>언제 쓰는가:</strong> {guide.use}</p><p><strong>경계:</strong> {guide.limits}</p><p className="mt-3 text-sm text-muted-foreground"><span className="mr-1">사람이 적음 · 판단은 검토 대상 ·</span><strong>소비처 근거:</strong> {guide.evidence}</p></section>
  </main>
}
