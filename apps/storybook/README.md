...
`test/stories.test.mjs`가 정적 빌드를 띄우고 모든 스토리를 Chromium으로 열어 넷을
재요.
...
| 한 줄에 선 컨트롤의 높이 | 스토리가 줄 요소에 `data-control-height="sm"`(또는 `md`, `lg`)를 달면, 그 줄의 컨트롤마다 화면에 그려진 높이를 재서 32px, 36px, 40px과 같은지 봐요. 줄의 `align-items`가 늘이기(stretch)면 짧은 컨트롤이 늘어나 어긋남이 가려지므로 실패해요 |
| 키보드 동작 | 스토리가 선언한 키를 그대로 눌러 보고, 포커스가 어디 있는지와 화면에 무엇이 적혔는지를 재요 |

측정 도구를 먼저 검증해요. 히트 영역과 컨트롤 높이 모두 알려진 기하를 렌더해
도구가 그 값을 그대로 읽지 못하면 스토리를 하나도 재기 전에 실패해요. 읽은 값을
믿으려면 도구를 먼저 믿을 수 있어야 하기 때문이에요.
