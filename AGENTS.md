...
**Phase 1이 닫혔다 (2026-09-11).** [ADR-0023](docs/adr/0023-second-generation-base-ui.md)이 2세대(Base UI)의 결정 전부를 담고, 완료 기록은 [`docs/handoff/phase-1-base-ui-map.md`](docs/handoff/phase-1-base-ui-map.md)다. 소비처 invest diary는 `@flameware/ui@0.2.3`·`@flameware/tokens@0.2.3`으로 돌고, 앱의 `components/ui/`·radix·cmdk·cva·shadcn 토큰 사본은 없다(investmentdiary#362). Phase 1 컴포넌트는 상태 표에서 전부 `stable`이다 — **깨는 변경은 major다.** **Phase 2 스펙은 [#317](https://github.com/flameware/massive-design/issues/317)이다** — ADR-0023 후보 목록을 앱의 실측 자리로 거른 범위(Alert warning·Progress·Empty state·NumberField·Pagination·ConfirmDialog 프리셋)와 Phase 1이 남긴 결손 7개(Menu.Label·Skeleton role·Badge nowrap·Combobox 후보 밖 값·README 색 개수·뷰포트 애드온·Foundations 표). 자리가 0인 후보(Popover·Toast·Radio·Switch·Accordion·ScrollArea·Stack/Grid·모션 토큰)는 Phase 3다. 티켓은 아직 갈리지 않았다 — `/to-tickets`가 다음 일이다. 그 밖의 이슈는 [`docs/agents/rules.md`](docs/agents/rules.md) 방법론 절의 셋 중 하나일 때만 연다.
