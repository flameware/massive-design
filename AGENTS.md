...
**Phase 1이 닫혔다 (2026-09-11).** [ADR-0023](docs/adr/0023-second-generation-base-ui.md)이 2세대(Base UI)의 결정 전부를 담고, 완료 기록은 [`docs/handoff/phase-1-base-ui-map.md`](docs/handoff/phase-1-base-ui-map.md)다. 소비처 invest diary는 `@flameware/ui@0.2.3`·`@flameware/tokens@0.2.3`으로 돌고, 앱의 `components/ui/`·radix·cmdk·cva·shadcn 토큰 사본은 없다(investmentdiary#362). Phase 1 컴포넌트는 상태 표에서 전부 `stable`이다 — **깨는 변경은 major다.** Phase 2 스펙은 아직 열리지 않았다. 후보는 ADR-0023 Phase 표(Popover·Toast·Radio·Switch·NumberField·Accordion·Collapsible·Progress·Meter·ScrollArea·Pagination·Stack/Grid·모션 토큰·Empty state·ConfirmDialog 프리셋·warning/info 패밀리)이고, 여는 조건은 [`docs/agents/rules.md`](docs/agents/rules.md) 방법론 절의 셋 중 하나다 — 소비처가 쓰는 모양·소비 앱의 결함·패키징. 그 밖의 이슈는 열지 않는다.
