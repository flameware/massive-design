...
**게시된 최신은 `0.6.4`다** — #463, 입력 계열의 쉬는 면을 `bg.surface`로 옮기고 회색은 무력화된 입력에만 남겼다. 등급과 근거, 그 앞의 버전들은 [`docs/releases.md`](docs/releases.md)에 있다 — 이 줄은 최신 하나만 말한다.
...
This file holds standing instructions and what is **open**. It is not the project's memory — that is what `docs/handoff/`, [`docs/releases.md`](docs/releases.md) and the issues are for. When a version is published, its paragraph goes to `docs/releases.md` and **the line above keeps only the newest version** — the release chain grew to six versions in one line before it was moved, which is the same work log this section exists to prevent. When a map closes:
