...
**게시된 최신은 `0.3.4`다** — `0.3.2`(누적분)·`0.3.3`(EmptyState aria 수정) 다음에 `0.3.4`가 소비처가 낸 세 자리를 담는다: `border.subtle`(#335) · Spinner `size="xl"`(#351) · ConfirmDialog `initialFocus`(#356). 셋 다 가산이라 patch다. 소비처 invest diary는 `@flameware/ui@0.3.4`·`@flameware/tokens@0.3.4`로 돈다. Phase 2가 새로 만든 다섯 컴포넌트(EmptyState·Progress·Pagination·NumberField·ConfirmDialog) 모두 상태 표에서 `stable`이다 — 다섯 다 앱에 착지한 증거가 있다.
