...
**게시된 최신은 `0.5.1`이다** — Select·Menu 팝업 항목의 쉬는 상태가 회색이던 것을 고쳤다(#387, #390). 면 사다리(떠 있으면 `bg-overlay`)가 Menu·Combobox까지 맞춰진 색 수정뿐이라 API는 그대로여서 patch로 올랐다. 소비처 invest diary는 `@flameware/ui@0.5.1`·`@flameware/tokens@0.5.1`으로 돈다.
