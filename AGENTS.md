...
**게시된 최신은 `0.6.0`이다** — 숲마루(#396) 온보딩이 드러낸 결손을 닫는다: 소비처가 brand 키 컬러 하나로 자기 브랜드를 입히는 `@flameware/tokens/ramp`의 `createBrandOverride`(#398), ADR-0026이 연 기본 폼 컨트롤 Switch·Slider·Radio/RadioGroup(#399·#400·#401), Icon을 서버 컴포넌트로 되돌린 것(#412), 소비처 온보딩 가이드(#403)까지 새 서브패스·새 API만 더하고 기존 계약은 그대로라 minor로 올랐다. brand 키를 주지 않으면 기존 brand(`#0f5fed`) 그대로라 기존 소비처 화면은 바뀌지 않는다. 그 앞 `0.5.2`는 Combobox 항목이 팝업 항목 중 유일하게 `.state` 층 밖에서 면을 직접 칠하던 것을 Select·Menu와 같은 모양으로 올렸다(#391, #393). 소비처 invest diary는 `@flameware/ui@0.6.0`·`@flameware/tokens@0.6.0`으로 돈다.
