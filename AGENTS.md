...
**게시된 최신은 `0.4.0`다** — Phase 3(#365)의 첫 슬라이스 #367이 `Table.Head`·`Table.Cell`에 `textAlign`·`numeric` 축을 열어 minor로 올랐다. 소비처 invest diary는 `@flameware/ui@0.4.0`·`@flameware/tokens@0.4.0`로 돈다. Phase 3의 나머지 진행은 #365를 본다. Phase 2가 새로 만든 다섯 컴포넌트(EmptyState·Progress·Pagination·NumberField·ConfirmDialog) 모두 상태 표에서 `stable`이다 — 다섯 다 앱에 착지한 증거가 있다.
