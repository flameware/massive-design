...
**Phase 2가 열렸다 — [#317](https://github.com/flameware/massive-design/issues/317)이 정본이다.** 범위는 ADR-0023 Phase 표를 invest diary의 **실측 자리**로 거른 것이고, 그 표도 티켓 목록도 이슈에 있다(여기로 옮기지 않는다). 자리가 0이어서 Phase 3로 내려간 후보: Popover·Toast·Radio·Switch·Accordion·Collapsible·Meter·ScrollArea·Stack/Grid·모션 토큰. 이슈를 여는 조건은 [`docs/agents/rules.md`](docs/agents/rules.md) 방법론 절의 셋 중 하나다 — 소비처가 쓰는 모양·소비 앱의 결함·패키징. 그 밖의 이슈는 열지 않는다.

**게시된 최신은 `0.3.0`이다** — 첫 티켓 [#320](https://github.com/flameware/massive-design/issues/320)이 `Menu.Group`·`Menu.GroupLabel`(별칭 `Label`)과 함께 올렸다. 소비처 invest diary는 아직 `@flameware/ui@0.2.3`·`@flameware/tokens@0.2.3`으로 돈다 — 앱의 `components/ui/`·radix·cmdk·cva·shadcn 토큰 사본은 없고(investmentdiary#362), `0.3.x` 착지는 [#333](https://github.com/flameware/massive-design/issues/333)이 거둔다.
